...
# --- CONFIGURAZIONE ---
st.set_page_config(page_title="Value Bet Safe v41", page_icon="🛡️", layout="wide")
st.title("🛡️ Calcolatore Strategico (Safe Mode v41)")
st.markdown("---")
...
with tab2:
    ...
    if df_future is None:
        ...
        if results:
            ...
            # --- FIX SICUREZZA VISUALIZZAZIONE ---
            # Definiamo le colonne che VORREMMO vedere
            desired_cols = ['datamecic', 'txtechipa1', 'txtechipa2', 'Bet', 'Quota', 'EV', 'ROI_Storico', 'Casi_Simili']
            # Filtriamo solo quelle che ESISTONO davvero nel dataframe (Safe Select)
            final_cols = [c for c in desired_cols if c in df_res.columns]
            
            st.dataframe(
                df_res[final_cols].style.format({
