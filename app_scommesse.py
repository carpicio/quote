...
def calculate_value(row, hfa=100):
    ...
    # Restituisce valori vuoti se mancano le quote
...
# --- INTERFACCIA ---
tab1, tab2 = st.tabs(["🔮 Calcolatore Partita Futura", "📊 Analisi CSV"])
...
with tab1:
    ...
    if st.button("Calcola Valore", type="primary"):
        dummy_row = {'elohomeo': elo_h_input, 'eloawayo': elo_a_input, 'cotaa': odd_1_input, 'cotae': odd_x_input, 'cotad': odd_2_input}
...
with tab2:
    ...
    if uploaded_file is not None:
        try:
            ...
            # Controllo se ci sono i risultati
            has_results = 'scor1' in df.columns and 'scor2' in df.columns and df['scor1'].notna().any()

            if has_results:
                def get_result(row):
                    try:
                        if pd.isna(row['scor1']) or pd.isna(row['scor2']): return np.nan
                        if row['scor1'] > row['scor2']: return '1'
                        elif row['scor1'] == row['scor2']: return 'X'
                        else: return '2'
                ...
                if not df_valid.empty:
                    ...
                    c1.metric("Profitto", f"{profit_1:.2f} u", delta=f"{roi_1:.2f}%")
                    c2.info("Strategia TRASFERTA (2)")
                    c2.metric("Profitto", f"{profit_2:.2f} u", delta=f"{roi_2:.2f}%")
            ...
            # --- CORREZIONE: Arrotondiamo direttamente i dati invece di usare .style ---
            df_display = df[cols].copy()
            cols_to_round = ['cotaa', 'cotad', 'Quota_Impl_1', 'Quota_Impl_2', 'EV_1', 'EV_2']
            # Arrotonda solo le colonne che esistono effettivamente
            valid_cols_to_round = [c for c in cols_to_round if c in df_display.columns]
            df_display[valid_cols_to_round] = df_display[valid_cols_to_round].round(2)
            
            st.dataframe(df_display)
            
            csv = df.to_csv(sep=';', decimal=',', index=False).encode('utf-8')
            st.download_button("Scarica CSV", data=csv, file_name="analisi_quote.csv", mime="text/csv")
